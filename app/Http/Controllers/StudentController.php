...
public function store(Request $request, Course $course)
{
    ...
    foreach ($lines as $line) {
        ...
        // Attempt to split on the first space; expecting format "LastName FirstName"
        $parts = explode(' ', $line, 2);
        if (count($parts) < 2) {
            // Not in "LastName FirstName" format, skip or handle differently
            continue;
        }
        $lastName = rtrim($parts[0], ','); // remove any trailing comma if present
        $firstName = $parts[1];

        // Check if a student with the same first and last name already exists for this course
        if (ClassStudent::where('class_id', $course->id)
